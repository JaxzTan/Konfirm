...
| 4 | ✅ **No Portal configuration exists for this.** The Enoki app config exposes only `allowedOrigins` / `authenticationProviders` / `domains` — there is no Move-call allowlist field. The allowlist is a **per-request argument** to `createSponsoredTransaction` (`allowedMoveCallTargets`), supplied by `allowedMoveCallTargets()` in `lib/enoki/sponsor.ts` and derived from `NEXT_PUBLIC_PACKAGE_ID`. It cannot drift from the deployed package: update the env var, rebuild, done | Enoki_setup.md §2.4 is wrong; verified in SDK `type.d.mts:62-75` | Move package deploy |
...
链上实测，不是推断。三个通过真实 attest 流程创建的 `Verdict`，以及创建它们的交易：

| Verdict | 发送者（zkLogin 用户） | 用户 SUI 余额 | Gas 付款方 |
|---|---|---|---|
| `0xd2778e87…` | `0xedbdf75b…` | **0** | `0x0dec4c7d…` |
| `0x526415ca…` | `0xe0b2050b…` | **0** | `0x0dec4c7d…` |
| `0x00c40416…` | `0x33a9868f…` | **0** | `0x0dec4c7d…` |

三个不同用户，余额全为 0，gas 全部由同一个**不属于他们**的地址支付 —— Enoki 的
赞助池。这一次同时坐实了验收标准 **3**（用户 0 SUI）、**4**（真实 digest + 对象
已创建）和 **5**（gas payer 不是用户）。本文档前面写的"第 3 和第 5 条同时成立才
说明赞助真的生效"，现在成立了。

| # | 验收项 | 状态 |
|---|---|---|
| 1 | `<GoogleLogin />` 弹出 Google 弹窗 | 未在浏览器点过，但链上已有三个不同 zkLogin 地址成功登录并签名 |
| 2 | 登录后返回 `0x...` 地址 | 同上 |
| 3 | 该地址余额 **0 SUI** | ✅ 实测 |
| 4 | `registry::create_verdict` 返回 digest | ✅ 实测，三笔 |
| 5 | gas payer **不是**用户地址 | ✅ 实测 |
| 6 | 同一 Google 账号重登得到**同一**地址 | ⬜️ 唯一还没证的一条 —— 四笔交易来自四个不同发送者，链上看不到重复登录。需要浏览器里登出再登入比对 |

**另一个发现**：Walrus testnet 的 blob 会过期。四个 Verdict 里已经有一个的
`trace_blob` 返回 404，推理记录永久丢失（链上记录不受影响）。`/v/[objectId]`
会降级显示并给出提示，但如果 demo 需要完整推理记录，评审前要用更长的存储周期
重新上传。
